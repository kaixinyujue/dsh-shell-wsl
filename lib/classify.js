...
export function isUsableWorkdir(workdir) {
    try {
...
export function isRunnerSpawnFailure(error, runnerProgram, workdir) {
    if (!isUsableWorkdir(workdir))
        return false;
    if (typeof error !== "object" || error === null)
        return false;
    const err = error;
    if (typeof err.code !== "string" || !EXECUTABLE_SPAWN_CODES.has(err.code))
        return false;
    if (typeof err.syscall !== "string")
        return false;
    const exactSyscall = `spawn ${runnerProgram}`;
    if (err.path === void 0)
        return err.syscall === exactSyscall;
    if (typeof err.path !== "string" || err.path.length === 0 || err.path !== runnerProgram)
        return false;
...
export function classifyRunnerFailure(exitCode, stderr, rules) {
    ...
    for (const rule of rules) {
        if (rule.allowedExitCodes !== void 0 && !rule.allowedExitCodes.includes(exitCode))
            continue;
 ...
 */
export const DOCKER_RUNNER_FAILURE_RULES = [
    {
...
export class WslContainerUnavailableError extends Error {
    constructor(detail) {
        super(`dsh-shell-wsl: the container runner is unavailable — ${detail} (the command did not run; fix the Docker setup, not the command)`);
